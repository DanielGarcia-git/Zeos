...
void init_task1(void)
{
  ...
  INIT_LIST_HEAD(&c->free_channel);
  for (int i = 0; i < MAX_CHANNELS; ++i) {
    if (i > 1) list_add_tail(&(c->table_channels[i].list), &c->free_channel);
    c->table_channels[i].m = UNUSED;
    c->table_channels[i].n_channel = i; //empieza en 2 porque la entrada estandard es I=0 O=1
  }
  c->table_channels[0].m = READ;
  c->table_channels[1].m = WRITE;
